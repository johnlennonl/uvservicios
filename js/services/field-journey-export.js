...
const EXCEL_SECTION_GROUPS = [
    ...
    { title: 'Sensor y presiones', fields: ['posee_sensor_fondo', 'descarga_datas_sensor', 'thp_psi', 'chp_psi', 'lf_psi', 'cond_chp', 'echometer', 'nivel_fluido_ft', 'sumergencia_ft', 'pip_echometer_psi', 'diagnostico'] },
...
const REPORT_COLUMN_MAP = new Map(REPORT_COLUMNS.map(([label, fieldName]) => [fieldName, { label, fieldName }]));
const EXCEL_EXPORT_COLUMNS = EXCEL_SECTION_GROUPS.flatMap(group => group.fields.map(fieldName => {
    const column = REPORT_COLUMN_MAP.get(fieldName);
    return column ? { ...column, groupTitle: group.title } : null;
}).filter(Boolean));
...
function normalizeJourney(journey = {}, records = []) {
    return {
        ...journey,
        fecha: journey.fecha || journey.journey_date || '',
        reportCount: Number(journey.reportCount || journey.total_reports || records.length || 0),
        firstHour: journey.firstHour || normalizeTime(journey.first_report_time),
        lastHour: journey.lastHour || normalizeTime(journey.last_report_time)
    };
}

function normalizeRecord(record = {}, journey = {}) {
    const payload = typeof record.raw_payload === 'object' && record.raw_payload ? record.raw_payload : {};
    ...
    return {
        ...
        fecha: payload.fecha || record.report_date || journey.fecha || journey.journey_date || '',
        hora: payload.hora || normalizeTime(record.report_time),
        pozo: String(payload.pozo || record.pozo || '').trim().toUpperCase()
    };
}

function sortJourneyRecords(records = [], journey = {}) {
    return records
        .map(record => normalizeRecord(record, journey))
        .sort((left, right) => {
            const byTime = String(left.hora || '').localeCompare(String(right.hora || ''));
            if (byTime !== 0) return byTime;
            return String(left.pozo || '').localeCompare(String(right.pozo || ''));
...
function buildJourneyPdfFileName(journey) {
    const parts = [
        'uvs-campo-pdf',
        sanitizeFileNameSegment(journey.locacion_jornada || 'jornada'),
        sanitizeFileNameSegment(journey.fecha || new Date().toISOString().slice(0, 10)),
...
function buildExcelDetailSheet(worksheet, journey, records) {
    ...
    worksheet.columns = EXCEL_EXPORT_COLUMNS.map(({ label, fieldName }) => ({ width: calculateExcelColumnWidth(label, fieldName, records) }));
    ...
    EXCEL_SECTION_GROUPS.forEach((group, index) => {
        const columnsForGroup = EXCEL_EXPORT_COLUMNS.filter(column => column.groupTitle === group.title);
        if (!columnsForGroup.length) return;
    ...
    worksheet.autoFilter = { from: { row: headerRowIndex, column: 1 }, to: { row: headerRowIndex, column: EXCEL_EXPORT_COLUMNS.length } };
    worksheet.eachRow((row, rowNumber) => {
        if (rowNumber >= headerRowIndex) row.height = rowNumber === headerRowIndex ? 34 : 24;
    });
    worksheet.getRow(1).height = 28;
    worksheet.getRow(2).height = 54;
    worksheet.getRow(3).height = 24;
...
function formatPreviewValue(value, fieldName) {
    ...
    return fieldName === 'estado_fosa_porcentaje' ? `${normalized} %` : normalized;
}
...
function buildPdfMarkup(journey, records) {
    const cards = records.map(record => `
        <article class="pdf-card">
            <div class="pdf-card-head">
                <div>
                    <span class="pdf-kicker">Vista estilo PDF</span>
                    <h2>${escapeHtml(String(record.pozo || '').toUpperCase())}</h2>
                    <p>${escapeHtml(record.campo || '--')} | ${escapeHtml(record.fecha || '--')} | ${escapeHtml(record.hora || '--')}</p>
                </div>
                <span class="pdf-status">${escapeHtml(record.estatus || '--')}</span>
            </div>
            ${WELL_PREVIEW_SECTIONS.map(section => `
                <section class="pdf-section">
                    <h3>${escapeHtml(section.title)}</h3>
                    <div class="pdf-grid">
                        ${section.items.map(([label, fieldName]) => `
                            <div class="pdf-item ${isLongPreviewField(fieldName) ? 'pdf-item-long' : ''}">
                                <strong>${escapeHtml(label)}</strong>
                                <span>${escapeHtml(formatPreviewValue(record[fieldName], fieldName))}</span>
                            </div>
                        `).join('')}
...
<head>
    <meta charset="UTF-8">
    <title>${escapeHtml(buildJourneyPdfFileName(journey))}</title>
    <style>
        body { font-family: Inter, Arial, sans-serif; margin: 0; padding: 24px; color: #0f172a; background: #f8fafc; }
        .pdf-shell { display: grid; gap: 18px; }
        .pdf-cover { background: linear-gradient(180deg, #ffffff, #ecfeff); border: 1px solid #cbd5e1; border-radius: 22px; padding: 24px; }
        .pdf-cover h1 { margin: 8px 0; font-size: 28px; }
        .pdf-cover p { margin: 0; color: #475569; line-height: 1.6; }
        .pdf-badges { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 16px; }
        .pdf-badge { display: inline-flex; padding: 8px 12px; border-radius: 999px; background: #ffffff; border: 1px solid #cbd5e1; font-size: 12px; font-weight: 700; }
        .pdf-card { background: #ffffff; border: 1px solid #cbd5e1; border-radius: 22px; padding: 20px; page-break-inside: avoid; }
        .pdf-card-head { display: flex; justify-content: space-between; gap: 12px; align-items: flex-start; margin-bottom: 16px; }
        .pdf-kicker { display: inline-flex; padding: 6px 10px; border-radius: 999px; background: #dbeafe; color: #1d4ed8; font-size: 11px; font-weight: 800; text-transform: uppercase; }
        .pdf-card h2 { margin: 10px 0 6px 0; font-size: 24px; }
        .pdf-card p { margin: 0; color: #64748b; }
        .pdf-status { display: inline-flex; padding: 8px 12px; border-radius: 999px; background: #fef3c7; color: #a16207; font-size: 12px; font-weight: 800; }
        .pdf-section { margin-top: 16px; }
        .pdf-section h3 { margin: 0 0 10px 0; font-size: 15px; }
        .pdf-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 10px; }
        .pdf-item { padding: 10px 12px; border: 1px solid #e2e8f0; border-radius: 14px; background: #f8fafc; }
        .pdf-item-long { grid-column: 1 / -1; }
        .pdf-item strong { display: block; font-size: 11px; text-transform: uppercase; color: #64748b; margin-bottom: 6px; }
        .pdf-item span { display: block; font-size: 13px; line-height: 1.6; }
        @media print {
            body { padding: 0; background: #ffffff; }
            .pdf-cover, .pdf-card { border-color: #d4d4d8; box-shadow: none; }
        }
    </style>
</head>
<body>
    <div class="pdf-shell">
        <section class="pdf-cover">
            <span class="pdf-kicker">Consolidado Campo</span>
            <h1>${escapeHtml(journey.locacion_jornada || 'Jornada sin locacion')}</h1>
            <p>${escapeHtml(journey.equipo_guardia || '--')} | ${escapeHtml(journey.fecha || '--')} | ${escapeHtml(journey.jornada || '--')}</p>
            <div class="pdf-badges">
                <span class="pdf-badge">${escapeHtml(String(journey.reportCount || records.length))} pozo(s)</span>
                <span class="pdf-badge">Ventana ${escapeHtml(journey.firstHour || '--')} a ${escapeHtml(journey.lastHour || '--')}</span>
                <span class="pdf-badge">${escapeHtml(journey.submitted_by_email || '')}</span>
            </div>
        </section>
        ${cards}
    </div>
...
export async function exportFieldJourneyToExcel(journeyInput = {}, recordsInput = []) {
    if (!window.ExcelJS) {
        throw new Error('La libreria de Excel no esta disponible en esta vista.');
    }

    const journey = normalizeJourney(journeyInput, recordsInput);
    const records = sortJourneyRecords(recordsInput, journey);
    if (!records.length) {
        throw new Error('La jornada no tiene pozos para exportar.');
    }
    ...
    const summarySheet = workbook.addWorksheet('Resumen', { views: [{ state: 'frozen', ySplit: 4 }] });
    const detailSheet = workbook.addWorksheet('Jornada Campo', { views: [{ state: 'frozen', ySplit: 6, xSplit: 4 }] });
    ...
    buildExcelSummarySheet(summarySheet, journey, records);
    buildExcelDetailSheet(detailSheet, journey, records);
...
export function openFieldJourneyPdf(journeyInput = {}, recordsInput = []) {
    const journey = normalizeJourney(journeyInput, recordsInput);
    const records = sortJourneyRecords(recordsInput, journey);
    if (!records.length) {
        throw new Error('La jornada no tiene pozos para exportar.');
    ...
    if (!printWindow) {
        throw new Error('El navegador bloqueo la ventana del PDF. Permite ventanas emergentes e intenta otra vez.');
    }

    printWindow.document.open();
    printWindow.document.write(buildPdfMarkup(journey, records));
